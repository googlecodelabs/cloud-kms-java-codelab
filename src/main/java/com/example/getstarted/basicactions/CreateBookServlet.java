...
public class CreateBookServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		req.setAttribute("destination", "create"); // The urlPattern to invoke
													// (this Servlet)
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// [START storageHelper]
		CloudStorageHelper storageHelper = (CloudStorageHelper) req.getServletContext().getAttribute("storageHelper");
		...
		// [START bookBuilder]
		Book book = new Book.Builder().author(req.getParameter("author")) // form
																			// parameter
				.description(req.getParameter("description")).publishedDate(req.getParameter("publishedDate"))
				.title(req.getParameter("title")).imageUrl(imageUrl).build();
		// [END bookBuilder]
		try {
			Long id = dao.createBook(book);
			resp.sendRedirect("/read?id=" + id.toString()); // read what we just
															// wrote
		} catch (Exception e) {
